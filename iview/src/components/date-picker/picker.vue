...
<template>
    ...
    >
        <div ref="reference" :class="[prefixCls + '-rel']" class="date-input-wrapper">
            <span v-if="showWeekDay" class="date-week-label">{{getWeekDay(internalValue[0])}}</span>
            <slot>
                <i-input
...
<script>
    ...
    export default {
        ...
        computed: {
            showWeekDay() {
                return this.showWeekLabel&&this.type == 'date'&&this.internalValue[0] != null
            },
            wrapperClasses(){
                return [prefixCls, {
