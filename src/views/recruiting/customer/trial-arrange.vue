<template>
	<Modal v-drag-modal v-model="modal$.show" :title="modal$.title" width="900" :mask-closable="false">				
		<div class="c-grid">
	    	<div class="box box-query">
	    		<Form ref="ref_search" :label-width="70" :model="search_field" class="filter-form row">
	    			...
	    			<Form-item :label="label_teacher" prop="eid" class="col-md-4 col-sm-6" v-if="type_arrange!='my'">
	    		...
	    		<div class="content">
					<div class="content-body" v-loading.like="'course_arranges'">
						<table class="modal-table">
							...
							<tr :class="currentRow==item?'ivu-table-row-highlight':''" v-for="(item,index) in data" @click="currentRow=item">
								<td class="b-r-0">
							...
							<tr v-if="!data.length">
								<td colspan="6"><div class="ivu-table-cell text-center">暂无数据</div></td>
							</tr>
						</table>		           
		            </div>
		...
		<div slot="footer">
			<div class="pull-left">已选择：{{labelText}}</div>
...
<script>
	...
	export default{
		...
		methods: {
			...
			save () {
				let uri = util.sprintf('course_arrange_students?ca_id=%s',this.currentRow.ca_id),
					params = {
						ca_id: this.currentRow.ca_id,
						list: []
					}
