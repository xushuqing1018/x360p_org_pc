	...
	<div class="c-grid">
		<div class="box box-query">
			<Form ref="ref_search" :model="search_field" :label-width="100" class="filter-form row">
				...
				<Form-item label="发起人" class="col-sm-3" prop="start_eid">
					<select-employee v-model="search_field.start_eid" placeholder="请选择发起人"></select-employee>
				</Form-item>
			...
			<Row class="basic">
                <Col span="24" class="mt-2" style="margin-left:0px;">                	
                	...
                	<CheckboxGroup class="ml-3" style="display: inline-block;" v-model="search_field.flow_result" prop="flow_result">
		...
		<div class="box box-result">
			<div class="toolbar">
				<Button class="ml-2" type="primary" icon="plus" @click="addInfo">发起流程</Button>
			</div>
			<div class="content">
...
<script>
	...
	export default{
		...
		data () {
			return {
				...
				column_render: {
					...
					start_bid (h,params) {
						return h('span',this.$filter('branch_name')(params.row.start_bid))    
					},
					is_finish(h,params){
						let tag =  h('Tag',{
								props:{
		...
		methods: {
			...
			check_flow (row) {
				this.$Modal.open('app/flow/check-flow-modal.vue',{
					props:{
                        fid:row.fid
                    },
					on:{
				...
				.then(modal=>{
					modal.show('审批【'+row.flow_title+'】流程')
				})
			}
		}
	}
