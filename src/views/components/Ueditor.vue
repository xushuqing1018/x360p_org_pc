...
export default {
  ...
  computed:{
    serverUrl(){
      return '/api/ueditor?token='+this.$store.state.user.token
    }
  },
  ...
  watch: {  	
  	value: function (value) {
  		// 绑定value更新后要同步设置组件内容
			if(this.value !== '' && this.scriptTagStatus === 2 && this.instance !== null && this.instance.isReady){	
        if(this.instance.getContent() != value){
  ...
  methods: {
    ...
    initEditor () {
      ...
      if (this.scriptTagStatus === 2 && this.instance === null) {
        ...
        this.$nextTick(() => {
          if(!this.ueditorConfig.serverUrl){
            this.ueditorConfig.serverUrl = this.serverUrl
          }
