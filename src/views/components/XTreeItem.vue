<template>
    <li role="treeitem"
        ...
        <div :class="anchorClasses" @click="handleItemClick" @mouseover="isHover=true" @mouseout="isHover=false">
            ...
            <span :class="model.color">{{model.text}}</span>
        </div>
        <ul role="group" ref="group" class="tree-children" v-if="isFolder">
            <x-tree-item v-for="(child, index) in model.children"
...
<script>
  ...
  export default {
    ...
    watch: {
      ...
      selIds (val) {    
        if (_.isArray(val)) {
          let index = val.indexOf(this.model.id)
          if(index===-1&&this.model.selected){          
    ...
    computed: {      
      ...
      anchorClasses () {
        return [
          ...
          {'tree-selected-part': this.model.selected_part},
          {'tree-hovered': this.isHover},
          {'tree-search': this.searchKey != '' && this.model.text.indexOf(this.searchKey) !== -1}
        ]
      },
      wholeRowClasses () {
        return [
    ...
    methods: {
      initSelected () {
      	...
      	if(selIds&&selIds.length>0){
      		let index = selIds.indexOf(this.model.id)
	      	if(index > -1&&!this.model.selected){
      			this.handleItemClick()
	      	}
      	}      	
