    	...
    	<div :class="classes">
    		<div class="list-main">
                <div class="list-condition">
                	<Row>
                		<Col span="8">
                			<Input v-model="search_field.org_name" size="small" placeholder="机构名称" v-focusme="'searchkey'"></Input>
                		</Col>
                		<Col span="10">
                			<ButtonGroup>
                ...
                <div class="list-body">
                    <div class="list-body-wrap" v-loading="'orgs'">
                        <table class="table-list">
                            <thead>
                                <tr>
                                    ...
                                    <th><div class="ivu-table-cell">机构名称</div></th>
                                    <th><div class="ivu-table-cell">域名</div></th>
                                    <th><div class="ivu-table-cell">电话</div></th>
                                    <th><div class="ivu-table-cell">账号状态</div></th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr :class="{'table-info':item.$selected}" @click="toggleSelected(item)" v-for="item in data">
                                    ...
                                    <td><div class="ivu-table-cell">
                                    	<Icon 
                                    		:type="item.is_frozen==1?'close-circled':'checkmark-circled'" 
                                    		:color="item.is_forzen==1?'#999999':'#006600'"
                                    		>
                                		</Icon>
                                		{{item.is_forzen==1?'已锁定':'已开启'}}
                                    </div></td>
                                </tr>
                                <tr v-if="data.length == 0">
                                    <td colspan="5" style="text-align: center"><p class="empty">没有合适的机构</p></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
export default {
    ...
    methods: {
		...
		getHost(item) {
			return util.sprintf('http://%s.%s',item.host,this.$store.state.user.info.product_url.split('//')[1])
		}
    },
    mounted() {
