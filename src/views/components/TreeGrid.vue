...
<template>
    <div :style="{width:tableWidth}" class='autoTbale'>
        <table class="table" id='hl-tree-table'>
            <thead>
                <tr>
            ...
            <tbody>
                <tr v-if="initItems && initItems.length>0" v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent}">
                    <td v-for="(column,snum) in columns" :key="column.key" :style="tdStyle(column)">
                        <label v-if="column.type === 'selection'">
                            <input type="checkbox" :value="item.id" v-model="checkGroup">
                        </label>
                        <div v-if="column.type === 'action'" class="btn_wrap">
                            <i-button 
                                class="action_btn" 
                                shape="circle" 
                                type="ghost" 
                                ...
                                :icon="action.icon" 
                                @click="btnClick(item,$event,index,action.type)" 
                                v-for="action in column.actions"
                                v-if="!((item.dpt_id == 0&&(action.type == 'edit'||action.type == 'delete')) || (item.dpt_type == 3&& action.type == 'add'))"
                                v-per="action.per"
                                :key="action.text">
                            </i-button>
                        </div>
                        <label @click="toggle(index,item)" v-if="!column.type">
                            <span v-if='snum==iconRow()'>
                                ...
                                <Icon type="ios-people" size="14" v-if="item.dpt_id==0"></Icon>
                                <Icon type="ios-home" size="14" v-else-if="item.dpt_type==1"></Icon>
                                <Icon type="location" size="14" v-else-if="item.dpt_type==2"></Icon>
                                <Icon type="ios-paper" size="14" v-else-if="item.dpt_type==3"></Icon>
                                <Icon type="network" size="14" v-else></Icon>
                            </span>
                            <span v-if="column.key=='dpt_type'">
                                <Tag v-if="index==0" type="border" color="red">机构</Tag>
                                <Tag v-else-if="item[column.key]==2" type="border" color="green">分公司</Tag>
                                <Tag v-else-if="item[column.key]==1" type="border" color="blue">校区</Tag>
                                <Tag v-else-if="item[column.key]==0" type="border">部门</Tag>
                                <Tag v-else-if="item[column.key]==3" type="border" color="green">项目</Tag>
                            </span>
                            <span v-else>{{renderBody(item,column,index)}}</span>
                        </label>
                    </td>
                </tr>
                <tr v-else>
                    <td colspan="3" style="text-align: center">{{emptyText}}</td>
                </tr>
            </tbody>
        </table>
    </div>
...
export default {
    ...
    methods: {
        ...
        // 点击某一行事件
        btnClick(data, event, index, text) {
            let result = this.makeData(data)
            this.$emit('btn-click', result, event, index, text)
        },
        // 点击事件 返回数据处理
        makeData(data) {
            const t = typeof(data);
