<template>
    <div class="calendar">
        <div class="calendar-tools">
            <span class="calendar-prev" @click="prev" v-if="switchMode">
            ...
            <span class="calendar-next"  @click="next" v-if="switchMode">
                <svg width="20" height="20" viewBox="0 0 16 16" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                <g class="transform-group">
                    <g transform="scale(0.015625, 0.015625)">
        ...
        <tr v-for="(day,k1) in days" style="{'animation-delay',(k1*30)+'ms'}">
            <td v-for="(child,k2) in day" :class="{'selected':child.selected,'disabled':child.disabled,'has-event':child.eventName!=undefined}" @click="select(k1,k2,child,$event)">
                ...
                <div class="text" :class="{'isLunarFestival':child.isLunarFestival,'isGregorianFestival':child.isGregorianFestival}" v-if="lunar && child.eventName==undefined">{{child.lunar}}</div>
            </td>
        </tr>
        </tbody>
        </table>
    ...
    methods: {
        ...
        render(y, m) {
            ...
            for (i = 1; i <= lastDateOfMonth; i++) {
                ...
                // 最后一行
                if (day == 6 && line<=4 && i!= lastDateOfMonth) {
                    line++
                }
        ...
        select(k1, k2, child,e) {
            ...
            if (this.range) {
                if (this.rangeBegin.length == 0 || this.rangeEndTemp != 0) {
                    ...
                    this.$emit('select',begin,end,child)
                }
                this.render(this.year, this.month)
            } else {
                // 取消上次选中
...
.calendar td.disabled.has-event span{
