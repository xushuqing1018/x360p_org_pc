<template>
  <div>
    ...
    <table class="modal-table">
      ...
      <tbody>
        <tr v-for="(item,index) in config.home_menu" :key="index" v-show="rid === 0 || item.rids.length == 0 || item.rids.indexOf(rid)!==-1">
          <td>
          ...
          <td>
            <div class="ivu-table-cell">
				...
				<div class="text-center" v-if="is_gm">
					<Icon type="ios-close-outline" @click="delMenu(index,config.home_menu)"></Icon>
					<Icon type="ios-arrow-thin-up" @click="upMenu(index,config.home_menu)" v-if="index>0"></Icon>
					<Icon type="ios-arrow-thin-down" @click="downMenu(index,config.home_menu)" v-if="index<menu_length-1"></Icon>
				</div>
            </div>
          </td>
		  <td>
            <div class="ivu-table-cell">
				<Select v-model="item.rids" multiple>
		...
		<tr v-if="is_gm">
			<td colspan="4">
				<div class="text-center">
					<Button type="default" long icon="plus" @click="addMenu(config.home_menu)">添加</Button>
				</div>
			</td>
		</tr>
      </tbody>
...
export default {
  ...
  methods: {
    ...
    initFinished() {
      this.rids = [];
      this.config.home_menu.forEach(m => {
        m.rids.forEach(r => {
  ...
  computed:{
	  menu_length(){
		  if(this.rid == 0){
			  return this.config.home_menu.length
		  }
		  let length = 0
		  this.config.home_menu.forEach(item=>{
