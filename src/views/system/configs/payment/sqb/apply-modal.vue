	...
	<Modal v-drag-modal v-model="modal$.show" :title="modal$.title" :width="750" :mask-closable="false">
		<div v-if="!hasAplay" class="aplay-wrap">
			<div style="font-weight:bold;margin-top:140px;line-height:30px;">
				亲，你还没有申请过~<br>
				<Button type="primary" icon="plus" @click="addSqbInfo">立即申请</Button>
			</div>
		</div>
		<div v-else>
			<Steps :current="currentStep" style="margin-top:20px">
				...
				<Step title='收钱吧审核中' v-if="!isSqbBack"></Step>
				<Step title='收钱吧审核驳回' v-if="isSqbBack" status="error"></Step>
				<Step title='审核成功' status="finish"></Step>
			</Steps>
			<div style="color:#f30;margin-top: 10px;" v-if="isBack||isSqbBack">驳回原因：{{info.client_apply_sqb_check[info.client_apply_sqb_check.length - 1].check_messages}}</div>
					...
					<TabPane label="商户基本信息">
						<div class="row">
							<Form-item label="商户名" prop="name" class="col-sm-6">
								<Input v-model="info.name" placeholder="至少有一个汉字" @on-blur="info.business_name=info.name" :readonly="!isEdit"></Input>
							</Form-item>
							<div class="col-sm-6"></div>
							<Form-item label="商户经营名称" prop="business_name" class="col-sm-6">
								<Input v-model="info.business_name" placeholder="商家实际在经营场所使用的名称，默认和商户名一样" :readonly="!isEdit"></Input>
							</Form-item>
							<div class="col-sm-6"></div>
							<Form-item label="联系人" prop="contact_name" class="col-sm-6">
								<Input v-model="info.contact_name" placeholder="联系人" :readonly="!isEdit"></Input>
							</Form-item>
							<div class="col-sm-6"></div>
							<Form-item label="联系电话" prop="contact_cellphone" class="col-sm-6">
								<Input v-model="info.contact_cellphone" placeholder="联系电话" :readonly="!isEdit"></Input>
							</Form-item>
							<div class="col-sm-6"></div>
							...
							<Form-item label="详细地址" prop="street_address" class="col-sm-6">
								<Input v-model="info.street_address" placeholder="详细地址" :readonly="!isEdit"></Input>
							</Form-item>
						</div>
					</TabPane>
					<TabPane label="结算账户信息">
						<div class="row overhide">
							...
							<Form-item label="银行卡号" prop="bank_card" class="col-sm-6">
								<Input v-model="info.bank_card" @on-blur="get_bank_name" placeholder="失去焦点后获取开户银行" :readonly="!isEdit"></Input>
							</Form-item>
							...
							<Form-item label="开户银行" prop="bank_name" class="col-sm-6">
								<Input v-model="info.bank_name" placeholder="开户银行" :readonly="!isEdit"></Input>
							</Form-item>
							...
							<Form-item label="开户支行" prop="branch_name" class="col-sm-6">
								<Select v-model="info.branch_name" filterable :disabled="!isEdit">
									<Option v-for="item in branch_list" :value="item">{{item}}</Option>
								</Select>
							</Form-item>
							<Form-item label="开户姓名" prop="holder" class="col-sm-6">
								<Input v-model="info.holder" placeholder="个人账户为开户人，企业账户为开户公司名" :readonly="!isEdit"></Input>
							</Form-item>
							<Form-item label="预留手机号" prop="bank_cellphone" class="col-sm-6">
								<Input v-model="info.bank_cellphone" placeholder="预留手机号" :readonly="!isEdit"></Input>
							</Form-item>
							<Form-item label="法人姓名" prop="legal_person_name" class="col-sm-6">
								<Input v-model="info.legal_person_name" placeholder="法人姓名" :readonly="!isEdit"></Input>
							</Form-item>
							...
							<Form-item label="工商注册号" prop="tax_payer_id" class="col-sm-6">
								<Input v-model="info.tax_payer_id" placeholder="企业账户需要传" :readonly="!isEdit"></Input>
							</Form-item>
							<Form-item label="证件类型" prop="id_type" class="col-sm-6">
								<Select v-model="info.id_type" :disabled="!isEdit">
							...
							<Form-item label="身份证号" prop="identity" class="col-sm-6">
								<Input v-model="info.identity" placeholder="身份证号" :readonly="!isEdit"></Input>
							</Form-item>
							<div class="col-sm-6"></div>
					...
					<TabPane label="其他">
						<div class="row">
							<Form-item label="备注" prop="remark" class="col-sm-6">
								<Input v-model="info.remark" placeholder="请输入备注" :readonly="!isEdit"></Input>
							</Form-item>
							<div class="col-sm-6"></div>
							<Form-item label="经度" prop="longitude" class="col-sm-6">
								<Input v-model="info.longitude" placeholder="请输入经度" :readonly="!isEdit"></Input>
							</Form-item>
							<div class="col-sm-6"></div>
							<Form-item label="纬度" prop="latitude" class="col-sm-6">
								<Input v-model="info.latitude" placeholder="请输入纬度" :readonly="!isEdit"></Input>
							</Form-item>
							<div class="col-sm-6"></div>
							<Form-item label="推广者" prop="user_id" class="col-sm-6">
								<Input v-model="info.user_id" placeholder="收钱吧提供的商户推广人id" :readonly="!isEdit"></Input>
							</Form-item>
							<div class="col-sm-6"></div>
							<Form-item label="外部商户号" prop="client_sn" class="col-sm-6">
								<Input v-model="info.client_sn" placeholder="外部商户号，可以根据此商户号查询商户信息" :readonly="!isEdit"></Input>
							</Form-item>
						</div>
					</TabPane>
				</Tabs>
		...
		<div slot="footer">
			...
			<Button type="primary" @click="SqbOk" v-if="hasAplay&&info.is_audit == 1&&info.is_check == 1" icon='edit'>重新提交</Button>
			<Button type="primary" v-if="hasAplay&&info.is_audit == 0&&info.is_check == 0" icon='more' disabled>正在审核</Button>
		</div>
	</Modal>
</template>
...
<script>
	...
	export default{
		...
		methods: {
			addSqbInfo() {
				this.$Modal.open('system/configs/payment/sqb/apply-info-modal.vue',{
					on: {
						save: () => {
							this.getSqbInfo()
						}
		...
		computed: {
			...
			isAgain() {
				...
				if(is_audit == 0 && is_check == 0&&this.info.client_apply_sqb_check.length > 1) {
					return true;
				}
				return false;
			},
			currentStep() {
				...
				if(is_audit == 0 && is_check == 0) {
					if(this.info.client_apply_sqb_check.length == 1) {
						stepIndex = 0;
					}
					else {
						stepIndex = 1;
