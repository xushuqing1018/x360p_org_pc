...
<script>
	...
	export default{
		...
		mounted () {
			...
			this.rest_api = `wxmp_materials?appid=${this.mpinfo.authorizer_appid}&type=${this.source_type}`	
			this.init_data()
		},
		methods: {	
			del (item) {
			...
			get_wx_files () {
				...
				this.$http.get(`wxmp_materials/sync?appid=${this.mpinfo.authorizer_appid}&type=${this.data_type}`)
				.then(response=>{
					this.init_data()
                    this.hidePageSpin()
				},response=>{
			...
			hook_get_param (params) {
				...
				let data = `${type}_data`
				this.rest_api = `wxmp_materials?appid=${this.mpinfo.authorizer_appid}&type=${type}`				
