<template>
	<div class="box">
		<div class="questionnaire-list" :style="{height:height+'px',overflow:'auto'}">
            ...
            <Button type="ghost" icon="plus" title="添加问卷" v-per="'ques.add'" @click="add"></Button>
            <div class="condition-list clearfix">
            	<div class="item" v-for="item in filterQuestionnaire">
		            <a :class="{'selected':current_item.qid == item.qid}" @click="selQuestionnaire(item)">
		            	...
		            	<ButtonGroup>
							...
							<Button 
								...
								title="添加条目" 
								v-per="'quesitem.add'"
								@click.stop="addQuestionnaireItem(item)">
							</Button>	
						</ButtonGroup>            	
		            </a>
		        </div>
		...
		<item-modal ref="item" @save="addItemSuccess"></item-modal>
	</div>
</template>
<script>
	...
	export default {
		...
		methods: {
			...
			addQuestionnaireItem(item) {
				this.operation_item = util.copy(item)
				this.$r('item')
				.set('questionnaire',item)
				.show(util.sprintf('添加问卷条目【%s】',item.name),'add')
			},
			deal_data(data) {
				if(data.list.length) {
