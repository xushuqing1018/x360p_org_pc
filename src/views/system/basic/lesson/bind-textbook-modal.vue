...
export default {
	...
	created() {
		this.rest_api = `lessons/${this.lid}/textbooks`;
	},
	mounted() {
		this.init_data();
	},
	methods: {
		addInfo() {
			this.$Modal.open('system/basic/lesson/bind-textbook-info-modal.vue', {
				on: {
					update: () => {
						this.init_data();
					}
		...
		deleteInfoAction(row) {
			let [method, url, msg] = ['delete', 'lesson_textbooks/' + row.ltb_id, '删除成功']
