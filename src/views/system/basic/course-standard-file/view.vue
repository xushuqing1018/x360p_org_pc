			...
			<Form :label-width="80" ref="form" :model="search_field" class="filter-form row">
				...
				<Form-item label="课标类型" prop="csft_did" class="col-md-3 col-sm-6">
					<Select v-model="search_field.csft_did">
						...
						<Option :value="item.did" v-for="item in dicts('course_standard_file_type')">{{item.title}}</Option>
					</Select>
				</Form-item>
                    ...
                    <div class="pull-right" v-if="is_orgs">
				        ...
				        <ButtonGroup>
				            <Button 
				                ...
				                size="small" 
				                icon="android-person" 
				                @click="type_value='local'">课标
				            </Button>
				            <Button 
				                :type="type_value=='center'?'primary':'ghost'" 
				                size="small" 
				                icon="android-people" 
				                @click="type_value='center'">总部课标
				            </Button>
				        </ButtonGroup>
					</div>
                </Col>                
        ...
        <div class="box box-result">
            <div class="toolbar">
                ...
                <Dropdown trigger="click" v-if="!(is_orgs&&type_value=='center')">
			        ...
			        <DropdownMenu slot="list">
			            <DropdownItem @click.native.stop="multiDel"><Icon type="ios-trash"></Icon> 删除</DropdownItem>
			            <DropdownItem @click.native.stop="multiDisable(1)"><Icon type="ios-checkmark"></Icon> 启用</DropdownItem>
			            <DropdownItem @click.native.stop="multiDisable(0)"><Icon type="minus-circled"></Icon> 禁用</DropdownItem>
			        </DropdownMenu>
			    </Dropdown>
                ...
                <RadioGroup v-model="type" type="button" style="float:right" @on-change="changeType" size="small">
            ...
            <div class="content">
                <div class="content-body">
                    <Row :gutter="32">
						<Col :lg="6" :md="8" :sm="12" v-for="(item,index) in csList" :key="index" style="margin-bottom: 20px;">
                            <div class="cs-bg" :class="{'cs-check-active':item.$check,'cs-check-mode':checkMode}" @click="changeCheckFromView(index)">
                                <dir class="cs-check-wrap">
                                    <Checkbox class="cs-check" v-model="item.$check" @on-change="changeCheck(index)"/>
                                </dir>
								<div class="cs-img" @click.native="opClick('view',item)">
                                    <img v-if="item.lesson_standard_file_item.length == 0" src="@/img/cs-none.png">
                                    <template v-else>
                                        <img v-if="item.lesson_standard_file_item[0].media_type == 'image'" :src="item.lesson_standard_file_item[0].file_url">
                                        <img v-else-if="item.lesson_standard_file_item[0].media_type == 'video'" :src="item.lesson_standard_file_item[0].file_url|video_cover_url_filter">
								...
								<div class="cs-content">
                                    ...
                                    <div class="cs-desc">
                                        课标类型：{{labelDicts(item.csft_did,'course_standard_file_type')}}<br>
                                        章节序号：<span v-if="item.lid">{{item.chapter_index}}</span>
                                        <span v-else-if="item.tbs_id">{{item.textbook_section.sort}}</span><br>
										<span v-if="item.textbook_section">章节名：{{item.textbook_section.section_title}}</span>
                                    </div>
                                    <div class="cs-status">
                                        <span v-if="item.enable"><Icon type="ios-checkmark" size="16" color="#00CC66"></Icon></span>
                                        <span v-else><Icon type="ios-minus" size="16" color="#FF1717"></Icon></span>
                                    </div>
                                    <div class="cs-op">
                                        <Dropdown trigger="click">
                                            ...
                                            <DropdownMenu slot="list">
												...
												<DropdownItem @click.native="opClick('view',item)">查看</DropdownItem>
                                                <DropdownItem @click.native="opClick('edit',item)" v-per="'course_standard_file.edit'" v-if="!(is_orgs&&type_value=='center')">编辑</DropdownItem>
                                                <DropdownItem @click.native="opClick('del',item)" v-per="'course_standard_file.delete'" v-if="!(is_orgs&&type_value=='center')">删除</DropdownItem>
