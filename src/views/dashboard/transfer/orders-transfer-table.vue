		...
		<table class="goods-table">
			<thead>
				<tr>
					...
					<th>单位</th>
					<th>购买单价</th>
					<th v-help="consume_help">
						课消情况
					</th>
					<th>剩余数量</th>
					<th v-help="transfer_help">结转情况</th>
					<th><span v-help="premium_help">结转溢价</span></th>
					<th>欠交金额</th>
				</tr>
			</thead>
			<tbody>
				<tr v-for="item in items" :class="item.line == 'even' ? 'table-tr-even': 'table-tr-odd'">
					...
					<td>
						<p>可结转金额：{{item.transfer_amount}}</p>
						<p>结转数量：
							<span>
								<InputNumber 
									v-model="item.transfer_num" 
									:min="0" 
						...
						扣除赠送:
							<span>
								<InputNumber 
									v-model="item.transfer_present_nums" 
									:min="0" 
					...
					<td>{{item.transfer_over_amount}}</td>
...
<script>
	...
	export default {
		...
		data() {
			this.consume_help = '计费单价:表示对已消耗的课如何计费<br/>标准价:此处打勾表示按原始单价计算课消，结转通常不享受缴费时的优惠，否则按购买单价(可修改)计算'
			this.transfer_help = '可结转金额 = 小计金额 - (课消数量 * 计费单价)<br/>结转金额 = 小计金额 - (购买数量 - 结转数量) * 计费单价'
			this.premium_help = '结转溢价 = (购买数量 - 结转数量) * (计费单价 - 购买单价)'
			return {
		...
		methods: {
			...
			billUnitPriceChange(item) {
				...
				this.cacuTransferAmount( item)
				if(!this.cacuAmount(item)) return false
				if(!this.cacuTransferOverAmonut(item)) return false			
			},
			// 结转数量改变
			remainNumsChange(item) {
				this.cacuAmount(item)
				this.cacuTransferOverAmonut(item)					
			},
			// 计算结转金额
			cacuAmount(item) {
				...
				let use_and_remain_nums = util.sub(item.nums, item.transfer_num)			// 除去结转的数量
				let use_and_remain = util.mul(use_and_remain_nums, item.bill_unit_price)	// 除去结转的总价
				item._amount = item.amount = util.float(util.sub(item.subtotal, use_and_remain).toFixed(6))			// 结转金额
				if(item.amount < 0) {
					item._amount = item.amount = '0.00'
				}
				item.transfer_price = util.float(util.div(item.amount, item.transfer_num).toFixed(6))
				return true
			},
			//  计算可结转金额
			cacuTransferAmount(item) {
				...
				item.transfer_amount = util.float(util.sub(item.subtotal, consume_amount).toFixed(6))   // 可结转金额
			},
			// 计算溢价
			cacuTransferOverAmonut(item) {
				if(item.transfer_num == 0) return false
				let use_and_remain_nums = util.sub(item.nums, item.transfer_num)			// 除去结转的数量
				let minus_price = util.sub(item.bill_unit_price, item.unit_price)		    // 计费单价与购买单价的差额
				item.transfer_over_amount = util.float(util.mul(use_and_remain_nums, minus_price).toFixed(6)) // 溢价金额
				return true
			},
			// 计费单价是否按标准价
