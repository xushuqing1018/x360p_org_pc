...
<script>
	...
	export default {
		...
		methods: {
			...
			// 数量改变重新统计原始金额、折扣金额
			numsChange(item) {
				let unit_price = item.origin_price 
				if(item._origin_price){
					unit_price = item._origin_price
				}else{
					item._origin_price = item.origin_price
				}
				if(item._step_price_define && item._step_price_define.length > 0){
					item.origin_price = item.price = util.get_step_unit_price_by_nums(item._step_price_define,item.nums,unit_price)
				}
