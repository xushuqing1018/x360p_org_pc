<template>
	<Modal v-drag-modal v-model="modal$.show" :title="modal$.title" :mask-closable="false" width="900">
		<Form ref="ref_search" :model="search_field" :label-width="70" class="filter-form row">
			<Form-item label="余额类型" prop="btype" class="col-sm-4">
				<Select v-model="search_field.btype" clearable>
		...
		data() {
			return {
				...
				column_render: {
					...
					btype(h,params) {
						return h('span',params.row.btype==1?'积分':'余额')
					},
					amount(h,params) {
						let tag = {color: 'red',label:'+ '+params.row.amount}
						if(params.row.op_type===3||params.row.op_type===4){
							tag.color = '#0c6'
