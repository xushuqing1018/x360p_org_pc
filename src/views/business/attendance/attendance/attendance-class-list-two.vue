    	...
    	<div class="box box-query">
    		<Form ref="ref_search" :label-width="70" :model="search_field" class="filter-form row">
    			...
    			<template v-if="searchExpand">	
                    ...
                    <Form-item label="是否补课" prop="is_makeup" class="col-md-3 col-sm-6">
	    				<Select v-model="search_field.is_makeup">
	    					<Option :value="-1">不限</Option>
	    					<Option :value="1">是</Option>
	    					<Option :value="0">否</Option>
	    				</Select>
	    			</Form-item>
                    <Form-item label="课消来源" prop="consume_source_type" class="col-md-3 col-sm-6">
	    				<Select v-model="search_field.is_makeup">
	    					<Option :value="-1">不限</Option>
	    					<Option :value="1">扣课时</Option>
	    					<Option :value="2">扣电子钱包金额</Option>
	    				</Select>
	    			</Form-item>
		...
		data () {
			return {
				...
				column_keys: {
					...
					elh_total_lesson_hours: {
						title: '总课时数',
						show: true
					},
					elh_total_lesson_amount: {
						title: '总金额',
						show: true
                    },
                    consume_source_type:{
				...
				column_render: {					
					cid (h,params) {
                        ...
                        if(params.row.student_name_list.length>0){								
                            obj_name += String(params.row.student_name_list.join(',')) 
                        }
                    ...
                    consume_source_type(h,params) {
                        let t = '扣课时'
                        if(params.row.consume_source_type == 2){
                            t = '扣余额'
                        }
                        return h('span',t)
                    }
